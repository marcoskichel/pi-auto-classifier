...
`src/index.ts`, `src/dev.ts`, or a rule file. Unit tests never load the extension, so a
...
- No comments in `src/index.ts`. `src/no-comments.ts` enforces it. A `ponytail:` prefix
